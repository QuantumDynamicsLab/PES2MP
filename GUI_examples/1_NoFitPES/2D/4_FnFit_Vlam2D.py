...
def fnfit_custom(x, a1,a2,a3,a4,a5,a6):
    import numpy as np
    return  a1*np.exp(-a4*x) + a2*np.exp(-a5*x) + a3*np.exp(-a6*x)

initial_val  =  [1e8, -1e7, 1e8, 4, 2, 1]                  # Enter initial guess
#-------------------------------------------------------------------------------#
...
# ------------------------   Plot parameters   ---------------------------------#
# plot parameters
R_lim  = [3,12]    # R limit for 1D/2D plots in Angstroms
...
start_pos = [0,2,4,3,8,0,2] # Use number greater than number of lambda terms (eg 1000)
#-------------------------------------------------------------------------------#
#cutoff   = 1000    # Energy cutoff in cm-1. (Not used: Using start pos) 
